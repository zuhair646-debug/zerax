"""
Tool Registry for the Zenrex Agent System.
...
async def web_fetch(url: str, max_chars: int = 30000) -> Dict[str, Any]:
    ...
    try:
        async with httpx.AsyncClient(timeout=12.0, follow_redirects=True) as client:
            r = await client.get(url, headers={"User-Agent": "Mozilla/5.0 ZenrexBot"})
...
- روابط الصوت فقط عبر `ZenrexQuran.audioUrl(reciterId, surahN, ayahN)` (تستخدم everyayah.com).
...
6. الضغط على .ayah-row → 
   ```
   const audio = new Audio(window.ZenrexQuran.audioUrl(activeReciter, {surah}, parseInt(this.dataset.ayah)));
   audio.play();
   ```
...
(function(){
  ...
  async function playAyah(ayahN, el){
    ...
    const url = (window.ZenrexQuran && window.ZenrexQuran.audioUrl)
      ? window.ZenrexQuran.audioUrl(activeReciter, SURAH_N, ayahN)
...
2. ضع هذين الـcomments داخل قسم المصحف:
   - <!-- ZENREX_QURAN_AYAHS -->  (سيُستبدل بـ {ayah_count} آية حقيقية)
   - <!-- ZENREX_QURAN_RECITERS --> (سيُستبدل بأزرار 14 قارئ)
...
تذكر: ضع <!-- ZENREX_QURAN_AYAHS --> و <!-- ZENREX_QURAN_RECITERS --> في قسم المصحف.
...
ابنِ الموقع الكامل الآن."""
    ...
    for attempt in range(3):
        try:
            ...
            if "<!-- ZENREX_QURAN_AYAHS -->" in html:
                html = html.replace("<!-- ZENREX_QURAN_AYAHS -->", blocks["ayahs_html"], 1)
            else:
            ...
            if "<!-- ZENREX_QURAN_RECITERS -->" in html:
                html = html.replace("<!-- ZENREX_QURAN_RECITERS -->", blocks["reciters_html"], 1)
            else:
