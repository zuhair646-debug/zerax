...
def _render_widget(surah: int, style_key: str = "classic", instance_id: str = "q1") -> str:
    ...
    return f"""
<div class="zenrex-qp" data-instance="{instance_id}" data-surah="{surah}" style="
  background:{style['bg']};
...
(function(){{
  var root = document.querySelector('.zqp-{instance_id}') ||
             document.querySelector('.zenrex-qp[data-instance="{instance_id}"]');
...
def inject_quran_players(html: str) -> str:
    ...
    def _rep(m):
        ...
        widget = widget.replace(
            f'<div class="zenrex-qp" data-instance="{inst}"',
            f'<div class="zenrex-qp zqp-{inst}" data-instance="{inst}"',
            1,
        )
        return widget
