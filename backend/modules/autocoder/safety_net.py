...
New approach: full freedom + tripwires:
  ...
  1. **Snapshot before any core-file write**
     Auto-backup to /tmp/zenrex_snapshots/<file>__<timestamp> so we can roll back.
...
REPO_ROOT = Path("/app")
SNAPSHOT_DIR = Path(os.environ.get("ZENREX_SNAPSHOT_DIR", "/tmp/zenrex_snapshots"))
...
✅ بدلاً منها، فيه **شبكة أمان تلقائية**:
  ...
  1. **Snapshot تلقائي** — قبل أي write_file لملف spine، النظام يحفظ نسخة
     في /tmp/zenrex_snapshots/ تلقائياً. ما تحتاج تطلبها.
