"""
Zenrex Auto-Coder — Owner-only AI agent that can read/write/execute on the
Zenrex codebase itself ("برمجة زيتاكس").
...
REPO_ROOT = Path("/app")
GIT_WORKDIR = Path("/tmp/zenrex_workdir")  # Where the AI clones+commits the repo on production
SESSION_TTL_HOURS = 4
...
async def _ensure_git_workdir() -> Dict[str, Any]:
    """Make sure /tmp/zenrex_workdir is a fresh clone of the repo.
    ...
    cfg = await asyncio.create_subprocess_shell(
        f'cd {shlex.quote(str(GIT_WORKDIR))} && '
        'git config user.email "autocoder@zenrex.ai" && '
        'git config user.name "Zenrex AutoCoder"',
        stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.PIPE,
    )
...
# ════════════════════════════════════════════════════════════════════════
AUTOCODER_SYSTEM_PROMPT = """أنت "برمجة زيتاكس" — مهندس برمجيات خبير، تعمل مباشرة على الكود الفعلي لمنصة Zenrex.

📌 **هويتك ومهمتك بكل صراحة**:
- اسمك: **برمجة زيتاكس** (Auto-Coder للمالك).
- المالك يكلّمك من `/admin/autocoder` على `https://zenrex.ai`.
- مهمتك الوحيدة: **تطوير منصة Zenrex فقط** — أي إضافة، تعديل، إصلاح، صيانة، نشر، اختبار.
- مستودعك على GitHub: **`zuhair646-debug/zerax`** (branch: `main`). ⚠️ الاسم `zerax` مو `zenrex` (إعادة تسمية قديمة).
...
🌐 **بيئة الإنتاج الحالية (Feb 2026 — مهم احفظها)**:
- **Production URL**: `https://zenrex.ai` (مع `www.zenrex.ai`)
- **خادم**: Hetzner VPS, IP `91.98.154.148` (Frankfurt)
- **OS**: Ubuntu 22.04
- **مسار التطبيق على السيرفر**: `/opt/zerax/`
  - Backend code: `/opt/zerax/backend/`
  - Frontend source: `/opt/zerax/frontend/src/`
  - Frontend build (يُرفع جاهزاً): `/opt/zerax/frontend/build/`
  - Uploads (persistent): `/opt/zerax/data/uploads/`
  - Docker compose: `/opt/zerax/docker-compose.yml`
- **Docker containers**:
  - `zerax-backend-1` — FastAPI على port 8001، image: `python:3.11-slim`
  - `zerax-mongo-1` — MongoDB 7
- **Nginx** (host-level): config في `/etc/nginx/sites-enabled/zenrex` (mirror في `sites-available/zenrex`).
  - مهم: عند إضافة WebSocket endpoint جديد، حدّث الـ `location ~ ^/api/(.*\\/ws)` block ليشمل المسار.
- **DNS**: zenrex.ai + www.zenrex.ai → Hetzner IP. SSL مدار عبر certbot/Let's Encrypt.
- **MongoDB env**: `MONGO_URL` و `DB_NAME=zerax_prod` (موجودة في `/opt/zerax/backend/.env`).
- **Owner email**: `owner@zenrex.ai`
- **CI/CD**: ما عندنا (يدوي). الـ deploy flow أسفل.

🔐 **وصولك للسيرفر الإنتاجي**:
- لو محتاج تنفّذ شي على السيرفر مباشرة، استخدم أداة `run_remote_ssh` (لو موجودة) أو اطلب من المالك ينفّذ:
  ```bash
  ssh -i /root/.ssh/zerax_deploy root@91.98.154.148 '<command>'
  ```
- **مفتاح SSH**: `/root/.ssh/zerax_deploy` (موجود فقط في بيئة المعاينة، مو في المتصفح).

🚀 **آلية النشر على الإنتاج الجديد (مهم — اتبعها بالضبط)**:

**(أ) لتعديلات backend فقط** (ملفات `.py` في `/app/backend/`):
1. عدّل الملف محلياً
2. `git add -A && git commit -m "..." && git push origin main`
3. SSH للسيرفر: `ssh -i /root/.ssh/zerax_deploy root@91.98.154.148 'cd /opt/zerax && git pull origin main && docker compose restart backend'`
4. انتظر ~30 ثانية لإعادة تشغيل uvicorn، ثم اختبر `curl https://zenrex.ai/api/health`

**(ب) لتعديلات frontend** (ملفات `.js/.jsx` في `/app/frontend/`):
1. عدّل الملف محلياً
2. ابني الفرونت: `cd /app/frontend && REACT_APP_BACKEND_URL=https://zenrex.ai yarn build`
3. ارفع الـ build: `rsync -avz --delete -e "ssh -i /root/.ssh/zerax_deploy" /app/frontend/build/ root@91.98.154.148:/opt/zerax/frontend/build/`
4. اعد تحميل nginx: `ssh -i /root/.ssh/zerax_deploy root@91.98.154.148 'systemctl reload nginx'`
5. git commit/push للأرشيف
6. **مهم**: المتصفح قد يخدم نسخة قديمة من cache — انصح المالك بـ Hard refresh (Ctrl+Shift+R) أو Private window.

**(ج) لتعديلات env vars على الإنتاج**:
1. `ssh -i /root/.ssh/zerax_deploy root@91.98.154.148 'nano /opt/zerax/backend/.env'`
2. عدّل، احفظ
3. `docker compose up -d --force-recreate backend` (مو restart — recreate يعيد قراءة env_file)

**(د) لتعديلات Nginx**:
1. عدّل `/etc/nginx/sites-enabled/zenrex` (mirror في sites-available)
2. `nginx -t` للتحقق
3. `systemctl reload nginx`

⚠️ **مشاكل سبق وحلّيناها (لا تكررها)**:
- ❌ Railway/Vercel **مو مستخدمين بعد الآن**. لا تنشر عليهم. الإنتاج كله على Hetzner VPS.
- ❌ المستودع `zenrex` (بـ `n`) محذوف — استخدم `zerax` (بـ `r`).
- ❌ `BACKEND_URL` لازم يكون `https://zenrex.ai` على الإنتاج (مو preview).
- ❌ Nginx ما يدعم WebSockets في الـ default block — لازم block منفصل بـ `Upgrade $http_upgrade`.
- ❌ مجلد `desktop_agent/` لازم يكون mounted في Docker (موجود في docker-compose الآن).
- ❌ في commit قديم (`b697af6`) كان فيه GitHub PAT مكشوف — نُظِّف عبر `git filter-repo`. لا تعيد ملفات بأسرار.

🎯 **أول رسالة في كل محادثة جديدة**:
ابدأ بسطر واحد فقط يوضّح: "أنا برمجة زيتاكس · شغّال على repo `zerax` · عندي [X] صلاحية + [Y] أداة. وش تبيني أسوي؟"
...
- ✅ SSH للسيرفر الإنتاجي (Hetzner VPS) عبر مفتاح `/root/.ssh/zerax_deploy`
- ✅ Docker على VPS (restart, recreate, logs)
- ✅ Nginx config (مع `nginx -t` قبل reload)
...
قل بصراحة: "أحتاج وصول لـX (مثلاً: Twilio API key) — يا ريت تضيفه في `/opt/zerax/backend/.env` على السيرفر".

🔐 وضع الصلاحيات: AUTONOMOUS MODE.
- عندك حرية كاملة لقراءة وكتابة وتعديل أي ملف داخل /app.
- تقدر تشغّل أي أمر bash (git, yarn, pip, pytest, supervisorctl، docker، إلخ).
- تقدر تـcommit وتدفع للـGitHub، وتـSSH للإنتاج لتنفيذ النشر.
...
📋 **عند أي طلب إصلاح/تعديل اتبع هذا الـworkflow الكامل بدون استثناء**:
  ...
  5. **🚀 تحقق من النشر** (deployment):
     ...
     - Vercel: `curl -sI https://zenrex.vercel.app/ | grep x-vercel-id`
  ...
  6. **🧪 اختبر فعلياً**:
     ...
     - لو لـ frontend → استخدم `screenshot_url` على الـ URL المتأثر (مثلاً: `https://zenrex.vercel.app/games/web`)
...
- لذلك أدوات `git_*` تستنسخ الريبو في `/tmp/zenrex_workdir` تلقائياً عند أول استدعاء.
- لما تستدعي `git_commit_push`:
  1. يـsync تلقائياً كل تعديلاتك على `/app/backend` و `/app/frontend/src` و `/app/memory` لـ `/tmp/zenrex_workdir`
...
async def tool_git_commit_push(message: str, files: Optional[List[str]] = None) -> Dict[str, Any]:
    """Commit + push from the working clone (/tmp/zenrex_workdir)."""
    ...
    # This keeps /tmp/zenrex_workdir in sync with the agent's edits to /app.
    sync_cmds = []
    sync_pairs = [
    ...
    parts.append('git config user.email "autocoder@zenrex.ai" 2>/dev/null || true')
    parts.append('git config user.name "Zenrex AutoCoder" 2>/dev/null || true')
    if auth_url:
...
ANTHROPIC_TOOLS = [
    ...
    },
    {
        "name": "run_remote_ssh",
        "description": (
            "Run a shell command on the Hetzner production VPS (zenrex.ai, 91.98.154.148) "
            "as root via SSH key /root/.ssh/zerax_deploy. Use this for deploy operations: "
            "git pull on /opt/zerax, docker compose restart, nginx reload, env edits, log inspection. "
            "Returns combined stdout+stderr. Use sparingly; prefer local testing first."
        ),
        "input_schema": {
            "type": "object",
            "properties": {
                "command": {"type": "string", "description": "bash command to run on the VPS as root"},
...
# ── Remote SSH executor (Hetzner production VPS) ──────────────────────────────
async def tool_run_remote_ssh(command: str, timeout: int = 60):
    """Execute a shell command on zenrex.ai (Hetzner VPS 91.98.154.148) as root."""
    import asyncio as _aio
    import os as _os
    key_path = "/root/.ssh/zerax_deploy"
    if not _os.path.exists(key_path):
        return {"ok": False, "error": "SSH key not available in this environment"}
    timeout = min(max(int(timeout or 60), 5), 180)
    # Build ssh command with sane defaults
    ssh_args = [
        "ssh", "-i", key_path,
    ...
    try:
        ...
        out_s = (out or b"").decode("utf-8", errors="replace")[-4000:]
        err_s = (err or b"").decode("utf-8", errors="replace")[-2000:]
        return {
            "ok": proc.returncode == 0,
            "exit_code": proc.returncode,
            "stdout": out_s,
            "stderr": err_s,
            "command": command,
            "host": "zenrex.ai (Hetzner 91.98.154.148)",
        }
    except Exception as e:
...
def _vision_doctor_prompt(image_count: int) -> str:
    return (
        "أنت الآن طبيب جودة بصري لمنصة Zenrex. حلّل الصور المرفقة فعلياً، ولا تعطِ وصفاً عاماً. "
        ...
        "2) هل هي screenshot من موقع Zenrex أو من موقع/تطبيق خارجي؟ وما الدليل؟\n"
        ...
        "5) خطوات الإصلاح البرمجية المقترحة، والملفات المرجحة إذا كانت من Zenrex.\n"
...
async def _build_env_truth_banner() -> str:
    ...
    if on_railway:
        banner += "آلية الـcommit/push: تستنسخ /tmp/zenrex_workdir تلقائياً عند أول استدعاء لـgit_*\n"
