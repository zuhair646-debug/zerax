...
REPO_ROOT = Path("/app")
GIT_WORKDIR = Path("/tmp/zerax_workdir")  # Where the AI clones+commits the repo on production
SESSION_TTL_HOURS = 4
...
async def _ensure_git_workdir() -> Dict[str, Any]:
    """Make sure /tmp/zerax_workdir is a fresh clone of the repo.
...
- المالك يكلّمك من `/admin/autocoder` على zerax.vercel.app.
- مهمتك الوحيدة: **تطوير منصة Zerax فقط** — أي إضافة، تعديل، إصلاح، صيانة، نشر، اختبار.
- مستودعك على GitHub: `zuhair646-debug/zerax` (branch: main).
- شغّال على Railway (`zerax-production.up.railway.app`) + Vercel (`zerax.vercel.app`).
...
📋 **عند أي طلب إصلاح/تعديل اتبع هذا الـworkflow الكامل بدون استثناء**:
  ...
  5. **🚀 تحقق من النشر** (deployment):
     ...
     - Vercel: `curl -sI https://zerax.vercel.app/ | grep x-vercel-id`
  ...
  6. **🧪 اختبر فعلياً**:
     ...
     - لو لـ frontend → استخدم `screenshot_url` على الـ URL المتأثر (مثلاً: `https://zerax.vercel.app/games/web`)
...
- لذلك أدوات `git_*` تستنسخ الريبو في `/tmp/zerax_workdir` تلقائياً عند أول استدعاء.
- لما تستدعي `git_commit_push`:
  1. يـsync تلقائياً كل تعديلاتك على `/app/backend` و `/app/frontend/src` و `/app/memory` لـ `/tmp/zerax_workdir`
...
async def tool_git_commit_push(message: str, files: Optional[List[str]] = None) -> Dict[str, Any]:
    """Commit + push from the working clone (/tmp/zerax_workdir)."""
    ...
    # This keeps /tmp/zerax_workdir in sync with the agent's edits to /app.
    sync_cmds = []
    sync_pairs = [
...
async def _build_env_truth_banner() -> str:
    ...
    if on_railway:
        banner += "آلية الـcommit/push: تستنسخ /tmp/zerax_workdir تلقائياً عند أول استدعاء لـgit_*\n"
