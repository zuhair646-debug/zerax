...
# ════════════════════════════════════════════════════════════════════════
# PRODUCTION SAFETY: Guards to prevent the AI from destroying itself
# ════════════════════════════════════════════════════════════════════════

# Files the AI is FORBIDDEN to modify on production (would brick the platform)
PROTECTED_PATHS = [
    "Dockerfile",
    "backend/Dockerfile",
    "frontend/Dockerfile",
    ...
    ".github/workflows/",
    "Procfile",
    "package.json",  # changing deps is risky on prod
    "yarn.lock",
    "package-lock.json",
    "backend/requirements.txt",
]

# Maximum git commits per chat session (prevents commit-spam loops)
MAX_COMMITS_PER_SESSION = 3


def _is_protected(path: str) -> bool:
    """Returns True if the path is in the protected list (prod-only)."""
    if not os.environ.get("RAILWAY_ENVIRONMENT"):
        return False  # only enforce on Railway production
    norm = path.replace("\\", "/").lstrip("/")
    if norm.startswith("app/"):
        norm = norm[4:]
    for prot in PROTECTED_PATHS:
        if norm == prot or norm.endswith("/" + prot) or norm.startswith(prot.rstrip("/") + "/"):
            return True
    return False
...
🛡️ **قواعد الأمان الحرجة (ضد التخريب الذاتي)**:

1. **❌ ممنوع تعدّل ملفات البنية التحتية** على production:
   - `Dockerfile` (أي مكان)
   - `railway.toml`, `railway.json`, `nixpacks.toml`, `Procfile`
   - `.github/workflows/`
   - `package.json`, `yarn.lock`, `package-lock.json`
   - `backend/requirements.txt`
   النظام يرفض هذي العمليات تلقائياً ويرجّع خطأ.

2. **❌ ممنوع loop الـcommits**: عندك حد أقصى **3 commits لكل جلسة**. لو فشل deploy، **توقّف فوراً** وأبلغ المالك بالخطأ. لا تجرّب fix-after-fix-after-fix.

3. **✅ القاعدة الذهبية**: لو طلب deploy فشل، **اقرأ السبب الحقيقي من logs أولاً** قبل ما تعدّل أي شي. 90% من الأخطاء تتحل بقراءة log واحد، مش بـ10 commits عشوائية.

4. **✅ لو فيه شك**: اسأل المالك. لا تخمّن. لا تحاول "كل الحلول الممكنة".

5. **✅ نطاق صلاحياتك**: عدّل كود التطبيق فقط (`/backend/modules/`, `/frontend/src/`, إلخ). البنية التحتية (Docker, CI, deps) محظورة.
...
async def tool_write_file(path: str, content: str) -> Dict[str, Any]:
    if _is_protected(path):
        return {"ok": False, "error": (
            f"🚫 ممنوع تعديل {path} على production — هذا ملف بنية تحتية "
            f"(Dockerfile/CI/dependencies). أي تعديل خاطئ يكسر النشر. "
            f"لو محتاج تعديل، اطلب من المالك صراحة."
        )}
    try:
        p = _resolve_path(path)
...
async def tool_edit_file(path: str, find: str, replace: str) -> Dict[str, Any]:
    if _is_protected(path):
        return {"ok": False, "error": (
            f"🚫 ممنوع تعديل {path} على production — ملف بنية تحتية محمي."
        )}
    try:
        p = _resolve_path(path)
        if not p.exists():
...
async def tool_delete_file(path: str) -> Dict[str, Any]:
    if _is_protected(path):
        return {"ok": False, "error": f"🚫 ممنوع حذف {path} — ملف بنية محمي."}
    try:
        p = _resolve_path(path)
        if not p.exists():
