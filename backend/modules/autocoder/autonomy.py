...
async def tool_browse_site(
    ...
    try:
        ...
        if action == "screenshot":
            import time
            path = f"/tmp/zenrex_screenshot_{int(time.time())}.png"
...
async def tool_create_test_user(
    ...
    if not email:
        ...
        email = f"test_{uuid.uuid4().hex[:8]}@zenrex.ai"
    # Check if exists
...
🌐 اختبار حقيقي للموقع (لما تعدّل واجهة):
   1. browse_site(action='navigate', url='https://zenrex-frontend.vercel.app')
