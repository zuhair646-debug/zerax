...
def _manifest(project: Dict[str, Any]) -> str:
    color = _safe_color(project.get("primary_color"))
    return json.dumps({
        "name": project.get("title") or "Zenrex App",
...
def _service_worker() -> str:
    return """const CACHE='zenrex-app-v1';
...
def _capacitor_config(project: Dict[str, Any]) -> str:
    app_id = "com.zenrex." + "".join(c for c in (project.get("title") or "app").lower() if c.isalnum())[:20] or "myapp"
    return json.dumps({
        "appId": app_id,
...
def _kotlin_app(project: Dict[str, Any]) -> str:
    name = project.get("title") or "MyApp"
    return f"""package com.zenrex.ai
...
def build_project(project: Dict[str, Any], features: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    if ptype == "native":
        ...
        _write("android/app/src/main/java/com/zenrex/app/MainActivity.kt", _kotlin_app(project))
