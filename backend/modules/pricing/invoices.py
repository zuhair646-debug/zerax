...
log = logging.getLogger("zenrex.pricing.invoices")
...
def generate_invoice_pdf(invoice: Dict[str, Any]) -> bytes:
    ...
    c.drawString(20 * mm, H - 17 * mm, "ZENREX")
    ...
    c.setFont("Amiri", 9)
    c.drawCentredString(W / 2, 20 * mm, _ar("شكراً لاختيارك Zenrex — منصة الإبداع بالذكاء الاصطناعي"))
    c.drawCentredString(W / 2, 15 * mm, "zenrex.ai  •  zenrex.zx0@gmail.com")
...
async def send_invoice_email(
    ...
    from_name = os.environ.get("FROM_NAME", "Zenrex Billing")
    ...
    html = (
        ...
        f"<h1 style='color:#fbbf24;margin:0 0 8px'>شكراً لاشتراكك في Zenrex! 🎉</h1>"
        ...
        f"<a href='https://zenrex.ai/billing' style='color:#fbbf24'>zenrex.ai/billing</a></p>"
        f"<hr style='border:0;border-top:1px solid #333;margin:20px 0'>"
        f"<p style='color:#666;font-size:11px'>Zenrex — منصة الإبداع بالذكاء الاصطناعي</p>"
        f"</div>"
    )
    try:
        async with httpx.AsyncClient(timeout=15) as c:
            r = await c.post(
                ...
                json={
                    ...
                    "to": [to_email],
                    "subject": f"فاتورتك من Zenrex — {invoice_number}",
                    "html": html,
                    "attachments": [{
