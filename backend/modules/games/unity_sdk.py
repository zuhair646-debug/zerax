...
🎮 Unity SDK exporter
═══════════════════════════════════════════════════════════════════════
Ships a downloadable .zip with Unity C# scripts that pull a Zitex
...
CSHARP_CLIENT = '''
// Zitex Unity Client — auto-generated. Drop into your Unity project.
...
namespace Zitex {
  [Serializable] public class ZitexAsset {
    public string id; public string type; public string subtype;
    public string name; public string url; public string cdn_url;
  }
  [Serializable] public class ZitexManifest {
    public string project_id;
    public string project_title;
    public List<ZitexAsset> images;
    public List<ZitexAsset> models;
    public List<ZitexAsset> voices;
    public string base_api;
    public string lora_url;
  }

  public static class ZitexClient {
    public static IEnumerator FetchManifest(string manifestUrl, Action<ZitexManifest> onReady, Action<string> onError = null) {
      using (var req = UnityWebRequest.Get(manifestUrl)) {
        yield return req.SendWebRequest();
        if (req.result == UnityWebRequest.Result.Success) {
          var m = JsonUtility.FromJson<ZitexManifest>(req.downloadHandler.text);
          onReady?.Invoke(m);
  ...
  public class ZitexImporter : MonoBehaviour {
    [Tooltip("URL to the project's manifest JSON")]
    public string manifestUrl;
    public Transform spritesParent;
    public Transform modelsParent;

    void Start() { StartCoroutine(ZitexClient.FetchManifest(manifestUrl, OnManifest)); }

    void OnManifest(ZitexManifest m) {
      Debug.Log($"[Zitex] loaded project: {m.project_title} ({m.images?.Count} imgs, {m.models?.Count} models)");
    ...
    IEnumerator LoadSpriteInto(ZitexAsset a) {
      yield return ZitexClient.LoadSprite(a.cdn_url ?? a.url, sprite => {
...
README_MD = '''# Zitex Unity SDK

Auto-generated SDK for your Zitex project. Imports all approved
images, 3D models (.glb), and voice lines into a Unity scene at
runtime.

## Setup (60 seconds)

1. **Copy the `Zitex/` folder** to your Unity project's `Assets/`.
2. **Install GLTFast** (for .glb models):
   - `Window > Package Manager > + > Add package by name`
   - `com.unity.cloud.gltfast`
3. **Create an empty GameObject** in your scene, name it `ZitexImporter`.
4. **Attach `ZitexImporter.cs`** to it.
5. **Set `manifestUrl`** to:
   `{MANIFEST_URL}`
6. Press Play — all approved assets stream in automatically.

## What's included
- `ZitexClient.cs`  — async helpers for fetching images/audio/models.
- `ZitexImporter.cs` — drag-and-drop scene component.
...
re-run `ZitexImporter` at any time to refresh the scene.
...
def build_zip(project: Dict[str, Any], base_api: str) -> bytes:
    ...
    with zipfile.ZipFile(buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
        zf.writestr("Zitex/ZitexClient.cs", CSHARP_CLIENT)
        zf.writestr("Zitex/manifest.json", json.dumps(manifest, indent=2))
