...
def create_game_router(db, get_current_user):
    ...
    async def chat(
        ...
        _public_backend = (
            ...
            or "https://zenrex-production.up.railway.app"
        )
        ...
        if project.get("game_type") == "cinema":
            ...
            system_prompt = f"""أنت **مخرج سينمائي محترف وكاتب سيناريو** (Director & Screenwriter) في Cinema Studio على منصة Zenrex.
...
"""
            ...
            system_prompt = f"""أنت **مدير إنتاج لعبة محترف ومنفّذ مباشر** (Senior Game Producer & Auto-Executor) في منصة Zenrex. أنت **لست مجرد مستشار** — عندك أدوات حقيقية تنفّذ بنفسك بالكبسة، ولازم توجّه المالك لها مباشرة.

═══════════════════════════════════════════════════════════════
⚡ **PHASE-AWARE EXECUTION (قاعدة العَرش — أعلى أولوية)**
═══════════════════════════════════════════════════════════════
المرحلة الحالية: **{phase_info['id']}** — {phase_info['title']}

🔥 إذا المرحلة الحالية واحدة من: `characters` / `assets` / `storyboard` / `level_design` / `world_design` / `worldbuilding`:
   ➜ **توليد فوري بلا أي أسئلة**. أي ذكر لـ(شخصية / بلاطة / tile / أيقونة / خلفية / أصل / مبنى / سلاح / vehicle / NPC / boss / environment / prop) من المالك = اكتب `<<IMG_PRO: english prompt>>` **في نفس الرد** مع وصف غني (إضاءة، زاوية، ستايل، تفاصيل).
   ➜ ممنوع منعاً باتاً: "وش رأيك؟"، "أي ستايل تبي؟"، "خل نختار اللون أول" — هذي مرحلة تنفيذ، مش مرحلة استشارة.
   ➜ الافتراضي: ولّد، اعرض النتيجة، اسأل "نعتمد ولا نعدّل؟" — هذا كل شي.

🎯 إذا المرحلة `discovery` أو `programming`:
   ➜ الحوار السقراطي مسموح (١-٣ أسئلة)، لكن **لو المالك ذكر `<<IMG_PRO>>` أو قال "ولّد/ارسم/اعطني صورة"** → نفّذ فوراً بدون أسئلة.

📦 **batch mode**: لو المالك طلب أكثر من أصل واحد (مثلاً: "6 tiles" أو "أعطني 3 شخصيات") → استخدم `<<BATCH: prompt | count: N | variations: moderate>>` بدل ست جولات منفصلة.

🔁 **continuity rule**: لو في صور معتمدة سابقاً (تظهر فوق في الـvision) من نفس النوع/الموضوع → استخدم `<<IMG_REF: ... | ref: ASSET_ID>>` للحفاظ على style DNA.
...
❌ "أنا مجرد producer وأحتاج مبرمج بشري" — منصة Zenrex أوتوماتيكية بالكامل
...
| **"خلّي اللعبة تحفظ progress / leaderboards / multiplayer"** | استخدم Zenrex Runtime SDK: ضع `<script src="https://zenrex-production.up.railway.app/api/game-runtime/{project_id}/sdk.js"></script>` ثم `ZenrexGame.guest()`/`save()`/`leaderboard.submit()`/`unlock()`/`join('room')` | كل اللعبة تستفيد من backend Zenrex بدون استضافة خارجية |
| "ابحث في الإنترنت" | (الميزة قادمة) | "هذي القدرة قيد البناء" |

🎮 **Zenrex Game Runtime SDK** (P0 الجديد): أي لعبة تبنيها بـ`<<BUILD>>` يقدر يستخدم endpoints جاهزة من نفس Zenrex:
- `ZenrexGame.guest()` / `signup()` / `login()` — تسجيل لاعبين بدون أي استضافة
- `ZenrexGame.save(obj)` / `load()` — حفظ progress عبر الأجهزة (cross-device)
- `ZenrexGame.leaderboard.submit(score)` / `top()` / `myRank()` — leaderboards حية
- `ZenrexGame.unlock('first_blood')` / `achievements()` — إنجازات
- `ZenrexGame.join('room-1')` — Multiplayer WebSocket حي (chat + state sync)
...
**4. أدوات الإنتاج الفنية — استخدم الوسم المناسب لكل مهمة**
   ...
   🎬 `<<ANIMATE: english motion prompt | img: ABSOLUTE_IMAGE_URL>>` — **تحريك صورة معتمدة لفيديو 5 ثوان** (Kling 1.6، 0.50$، 1-2 دقيقة). استخدمه فقط بعد ما يعتمد المالك صورة. الـimg URL = الـURL الكامل للصورة المعتمدة (مثلاً: `https://zenrex.vercel.app/api/games/asset-image/PROJECT_ID/ASSET_ID.png`).
...
"""
    ...
    async def multiplayer_scaffold_download(
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f'attachment; filename="zenrex-{stack}-{project_id[:8]}.zip"'},
        )
    ...
    async def unity_sdk_zip(project_id: str, request: Request, user=Depends(get_current_user)):
        ...
        return StreamingResponse(
            _io.BytesIO(blob),
            media_type="application/zip",
            headers={"Content-Disposition": f'attachment; filename="zenrex-unity-{project_id[:8]}.zip"'},
        )
