...
- 🎙️ **التعليق الصوتي يشتغل بدون أي مفتاح إضافي**: `generate_voiceover` يستخدم OpenAI TTS مباشرة (مفتاحه عندنا أصلاً) ويدعم **كل اللغات** بما فيها الكورية، اليابانية، العربية، الإنجليزية، الفرنسية، التركية، الأوردو. لا تطلب من العميل مفتاح ElevenLabs إلا لو هو طلب صراحةً أصوات ElevenLabs المخصصة (clone خاص). لاستخدام OpenAI TTS، استخدم voice_id من: `onyx, echo, fable, alloy, nova, shimmer` (6 أصوات متعددة اللغات).
...
**المرحلة 2 — تأسيس الشخصيات (`characters`):**
   ...
   لكل شخصية → `generate_image` ببرومبت تفصيلي بأسلوب النوع المختار (كرتون/أنمي/واقعي/إلخ).
   احفظ كأصول `kind=character` ليبقى الشكل ثابتاً عبر كل اللقطات.
   هذي خطوة **حرجة** — تمنع تغير وجوه الشخصيات بين اللقطات.

**المرحلة 3 — السيناريو (`script`):**
   `write_script` بسيناريو مفصّل بناءً على الشخصيات والنوع.
   اعرضه بـ markdown واطلب الموافقة.
...
**المرحلة 5 — اللقطات/الستوري بورد (`storyboard`):**
   ...
   `generate_storyboard` بنفس أسلوب الشخصيات من المرحلة 2.
