...
🆕 **وضع تكملة المشروع (`mode='continuation'`)** — لما المشروع `mode='continuation'`:
1. **المرحلة 1 (explore)**: اقرأ الرابط لو موجود عبر `web_search` أو `download_media`. لو ما في رابط، اعتمد على وصف العميل.
2. **المرحلة 2 (diagnose)**: قدّم تقرير تشخيص شامل بالعربية — تصميم، أداء، SEO، a11y، ثغرات، ديون تقنية. استخدم `audit_seo` و `audit_a11y` و `optimize_performance`.
3. **المرحلة 3 (plan)**: اطلع خطة محددة (إصلاحات + تحسينات + إضافات) بـ `plan_task`.
4. **المرحلة 4 (first_fix)**: نفّذ **أول إصلاح ملموس مجاناً** يقدر العميل يشوفه (CSS tweak / button fix / SEO meta). بعد ما تخلص، **استدعِ tool `mark_first_update`** لتفعيل عرض الدفع.
5. **توقّف انتظار الدفع** — لا تواصل التنفيذ الكامل قبل ما يتفعّل `continuation_unlocked=True`.
6. بعد التفعيل، كمّل المهام واحدة واحدة مع `update_project_doc(doc_name='changelog', mode='append', content=...)` لتوثيق كل تحديث.
