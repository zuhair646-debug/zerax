...
export default function AppGamesStudio({ user }) {
  ...
  // 🗑️ Soft-delete asset (parity with Web)
              ...
              <div className="max-w-3xl mx-auto space-y-4">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div>
                    ...
                    <p className="text-xs text-zinc-400 mt-1">اضغط "ابني وانشر اللايف" والـ AI يبني التطبيق كامل ويطلق رابط حي.</p>
                  </div>
                  <BuildLiveButton
                  ...
                  />
                  {project?.preview_url && <QAAnalyzeButton projectId={project?.id} accentColor="blue" />}
                  <ProStudioTools projectId={project?.id} projectTitle={project?.title} accentColor="blue" />
                </div>
                {project?.preview_url ? (
                  ...
                  <div className="rounded-xl border border-white/10 bg-black/40 p-8 text-center">
                    ...
                    <p className="text-xs text-zinc-500">اضغط الزر فوق وراح يبني AI التطبيق ويطلق لك رابط حي خلال 60-180 ثانية.</p>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4" data-testid="tab-content-chat">
            ...
            {messages.map((msg, i) => (
              <div key={i} className="space-y-3">
                ...
                {/* 🎨 Generated assets — SEPARATE card below chat bubble */}
                {msg.generated_assets?.length > 0 && (
                  <div className="mt-2 space-y-3 pl-2 pr-2 md:pl-4 md:pr-4" data-testid="inline-assets">
                    {msg.generated_assets.map((a) => {
                      ...
                      return (
                        <div key={a.id} className="border-2 border-blue-500/40 rounded-xl overflow-hidden bg-gradient-to-br from-blue-500/5 to-indigo-500/5 shadow-lg shadow-blue-500/10">
                          {a.safety_net && (
                            <div className="px-3 py-1.5 bg-blue-500/15 border-b border-blue-500/30 text-[10px] text-blue-200 flex items-center gap-1.5">
                              <span>🛟</span>
                              <span>تم التوليد التلقائي (Safety-Net)</span>
                            </div>
                          )}
                          {isImg && fullUrl && (
                            <SafeAssetImage
                          ...
                          {isVid && fullUrl && (
                            <video src={fullUrl} controls className="w-full max-w-2xl"
                                   data-testid={`generated-asset-${a.id}`} />
                          )}
                          {isAudio && fullUrl && (
                            <div className="p-3 flex flex-col gap-2" data-testid={`generated-asset-${a.id}`}>
              ...
              <div key={type} className="mb-4">
                ...
                <div className="space-y-2">
                  {visibleAssets.map(asset => (
                    <div key={asset.id} className={`relative p-3 rounded-lg border ${
