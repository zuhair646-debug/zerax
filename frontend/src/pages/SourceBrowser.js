...
export default function SourceBrowser({ user }) {
  ...
  const downloadZipAll = async () => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `zenrex-${new Date().toISOString().slice(0, 10)}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      setTimeout(() => URL.revokeObjectURL(url), 5000);
  ...
  const downloadZipFolder = async (prefix) => {
    try {
      ...
      const a = document.createElement('a');
      a.href = url;
      a.download = `zenrex-${prefix.replace(/\//g, '-').replace(/-$/, '')}.zip`;
      document.body.appendChild(a);
      a.click();
      document.body.removeChild(a);
      setTimeout(() => URL.revokeObjectURL(url), 5000);
      ...
      <div className="max-w-5xl mx-auto p-4 md:p-8">
        <div className="mb-6">
          ...
          <p className="text-sm text-white/60 leading-relaxed">
            انسخ كل ملف إلى مستودعك على GitHub (<code className="text-yellow-300">zuhair646-debug/zenrex</code>) محافظاً على نفس المسار.
