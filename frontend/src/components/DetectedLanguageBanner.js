 ...
 * Listens for the custom `zenrex:lang-auto-detected` event (dispatched by
...
export default function DetectedLanguageBanner() {
  ...
  useEffect(() => {
    const handler = (e) => {
      ...
      try { if (sessionStorage.getItem('zenrex_geo_banner_seen') === '1') return; }
      catch (_) { /* */ }
      setDetected(code);
      setShow(true);
    ...
    window.addEventListener('zenrex:lang-auto-detected', handler);
    return () => window.removeEventListener('zenrex:lang-auto-detected', handler);
  }, []);
  ...
  const keepArabic = () => {
    ...
    i18n.changeLanguage('ar');
      try { sessionStorage.setItem('zenrex_geo_banner_seen', '1'); } catch (_) { /* */ }
    setShow(false);
  };
