...
export const Navbar = ({ user, transparent = false, setUser }) => {
  ...
  const [promoShown, setPromoShown] = React.useState(
    () => typeof window !== 'undefined' && sessionStorage.getItem('zitex_promo_dismissed') !== '1'
  );
  ...
  React.useEffect(() => {
    ...
    window.addEventListener('zitex:promo-change', onPromoChange);
    return () => window.removeEventListener('zitex:promo-change', onPromoChange);
  }, []);
  ...
  return (
    <nav
      className={`fixed left-0 right-0 z-50 ${transparent ? 'bg-[#0a0a12]/95 backdrop-blur-xl border-b border-amber-500/10' : 'bg-[#0a0a12]/98 backdrop-blur-md border-b border-amber-500/20'}`}
