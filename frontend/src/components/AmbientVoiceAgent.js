...
const WAKE_PATTERNS = [/ز[يَ]ت[كك]س/i, /zenrex/i, /يا\s+زيتكس/i];
...
function getUserName() {
  ...
  return localStorage.getItem('zenrex_user_name') || '';
}
...
function getUserGender() {
  ...
  const stored = (localStorage.getItem('zenrex_user_gender') || '').toLowerCase();
...
export default function AmbientVoiceAgent() {
  ...
  const toggleMic = useCallback(() => {
    const next = !micOn;
    setMicOn(next);
    try { localStorage.setItem('zenrex_mic_on', next ? '1' : '0'); } catch (_) {}
    if (!next) {
  ...
  useEffect(() => {
    try {
      if (localStorage.getItem('zenrex_mic_on') === '1') setMicOn(true);
    } catch (_) {}
  }, []);
